...
class VNode {
  ...
  flatten({
    condition = node => node instanceof VElement
  } = {}) {
    let elements = [];
  ...
  toString() {
    return `${this.nodeName}`; // return `${this.nodeName}: ${JSON.stringify(this.nodeValue)}`
  }
...
class VElement extends VNodeQuery {
  ...
  get classList() {
    let self = this;
    let classNames = (this.className || '').trim().split(/\s+/g) || []; // log('classList', classNames)
  ...
  render(h = html) {
    return h(this.tagName.toLowerCase(), this.attributes, this.childNodes.map(c => c.render(h)));
  }
...
export { CDATA, VDocument, VDocumentFragment, VElement, VHTMLDocument, VNode, VNodeQuery, VTextNode, createDocument, createHTMLDocument, document, escapeHTML, escapeRegExp, h, html, parseHTML, unescapeHTML, vdom, xml };
